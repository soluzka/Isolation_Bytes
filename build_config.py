...
def _ensure_spec_excludes(spec_path, modules):
    """Repair a PyInstaller spec before invoking PyInstaller.

    Spec files must carry exclusions and hook paths inside Analysis()/the spec;
    PyInstaller rejects --exclude-module when a .spec file is supplied.
    This function is intentionally tolerant of specs generated by older
    versions of this build script, including malformed hookspath entries.
    """
    ...
    # Ensure every requested module is present in Analysis(excludes=[...]).
    excludes_match = re.search(r'(?m)^\\s*excludes\\s*=\\s*\\[([^]]*)\\],?', content)
    if excludes_match:
        existing = excludes_match.group(1)
        missing = [m for m in wanted if not re.search(
            rf"(?<![\\w.]){re.escape(m)}(?![\\w.])", existing
        )]
        if missing:
            addition = (existing.rstrip() + ', ' if existing.strip() else '') + ', '.join(
                repr(m) for m in missing
            )
            content = content[:excludes_match.start(1)] + addition + content[excludes_match.end(1):]
            print(f'Added PyInstaller spec exclusions: {", ".join(missing)}')
    else:
    ...
    # Repair/normalize hookspath. Older generated specs may contain
    # hookspath=[, r'...'], which is invalid Python.
    hook_dir = os.path.join(BASE_DIR, 'pyinstaller_hooks')
    hook_entry = repr(hook_dir)
    hook_match = re.search(r'(?m)^\\s*hookspath\\s*=\\s*\\[([^]]*)\\]', content)
    if hook_match:
        existing = hook_match.group(1).strip()
