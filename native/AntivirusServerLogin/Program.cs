...
public class LoginForm : Form
{
    ...
    public void LaunchApp()
    {
        ...
        try
        {
            var psi = new ProcessStartInfo
            {
                ...
                Arguments = $"shell:AppsFolder\\{Global.AUMID}",
                UseShellExecute = true
            };
            Process.Start(psi);
    ...
    private void StartNetworkAgent()
    {
        try
        {
            ...
            {
                FileName = agentExe,
                Arguments = $"--server {Global.SERVER_URL} --key=af5caf8d3d8080f8a8686f72dfa52eb92dad9ef95ebfdcd6f6494cf99ed6b909 --auto-start",
                UseShellExecute = false,
                CreateNoWindow = true,
                WorkingDirectory = Path.GetDirectoryName(agentExe) ?? agentDir
            };
            Process.Start(psi);
        }
        catch
