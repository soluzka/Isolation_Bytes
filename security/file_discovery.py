...
_DEFAULT_DB = os.path.join(tempfile.gettempdir(), "Isolation_Bytes", "file_discovery.sqlite3")


class FileDiscoveryLedger:
    """Thread/process-safe ledger of exact file identities and security evidence."""

    def __init__(self, db_path: Optional[str] = None):
        self.db_path = os.path.abspath(db_path or os.environ.get(_DB_ENV, _DEFAULT_DB))
    ...
    def _initialize(self):
        with self._lock, self._connect() as db:
            ...
            """)
            db.execute("CREATE INDEX IF NOT EXISTS idx_file_observations_path ON file_observations(path)")
    ...
    def lookup(self, sha256: str) -> Optional[Dict[str, Any]]:
        with self._lock, self._connect() as db:
            row = db.execute("SELECT * FROM file_observations WHERE sha256 = ?", (sha256.lower(),)).fetchone()
            if row is None:
                return None
            columns = [item[1] for item in db.execute("PRAGMA table_info(file_observations)").fetchall()]
