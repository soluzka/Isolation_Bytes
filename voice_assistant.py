...
# are executed on the local agent instead of the VPS.
# ---------------------------------------------------------------------------
_voice_command_queue = {}
...
def _complete_voice_command(device_id, job_id, status, result):
    ...
    record = _voice_command_queue.get(device_id, {}).get(job_id)
    if not record:
        return False
...
def _prune_voice_command_queue(max_age=300):
    ...
    now = time.time()
    for device_id in list(_voice_command_queue.keys()):
        queue = _voice_command_queue[device_id]
        for job_id in list(queue.keys()):
            record = queue[job_id]
            if record.get('status') in ('completed', 'error'):
                if record.get('finished', 0) + max_age < now:
                    del queue[job_id]
        if not queue:
            del _voice_command_queue[device_id]
...
def _is_authenticated():
    """Accept either an admin session ('logged_in') or a licensed user session ('user_logged_in')."""
