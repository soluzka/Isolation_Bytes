...
def _create_default_env(env_path):
    ...
    try:
        ...
        if os.name == 'nt':
            username = f"{os.environ.get('USERDOMAIN')}\\{getpass.getuser()}" if os.environ.get('USERDOMAIN') else getpass.getuser()
            _subprocess.run([
    ...
    <body>
        <div class="card">
            ...
            <div class="info">
                ...
                {'<span class="activated">Auto-activated for this device</span>' if activated else '<span class="not-activated">Activate manually on the next page</span>'}
            </div>
            ...
            <a class="btn" href="/?page=activate&machine_id={machine_id}">Go to Activate / Login</a>
...
if (Test-Path $agentExe) {
    $apiKey = 'af5caf8d3d8080f8a8686f72dfa52eb92dad9ef95ebfdcd6f6494cf99ed6b909'
...
"%LOCALAPPDATA%\IsolationBytes\IsolationBytesAgent.exe" --server https://isolation-bytes.com --key=af5caf8d3d8080f8a8686f72dfa52eb92dad9ef95ebfdcd6f6494cf99ed6b909
'''
...
def cloud_checksums():
    ...
    for f, (fp, mtime, size) in files_info.items():
        try:
            ...
            pass
    result = {'version': '1.8.944.0', 'files': checksums}
...
@cloud_bp.route('/quarantine', methods=['GET'], endpoint='quarantine')
@cloud_bp.route('/quarantine.html', methods=['GET'])
def cloud_quarantine_page():
    files = _cloud_list_quarantine_files()
    # Also collect quarantined files from connected agents
    agent_qfiles = []
    # Collect blocked ransomware/persistence findings for quarantine option
    blocked_findings = []
    agents = _all_agents()
    for device_id, ag in agents.items():
        host = ag.get('hostname', device_id)
        for qf in (ag.get('quarantine_files') or []):
            qf = dict(qf)
            qf['hostname'] = host
            qf['device_id'] = device_id
            agent_qfiles.append(qf)
        # Collect ALL blocked findings (ransomware, persistence, yara, ml)
        # from last_report AND from the blocked_files registry
        seen_paths = set()
        last_report = ag.get('last_report') or {}
        for f in (last_report.get('findings') or []):
            ...
            if f.get('blocked') or ttype in ('ransomware', 'persistence', 'yara_match', 'ml_suspicious'):
                ...
                blocked_findings.append({
                    ...
                    'device_id': device_id,
                    'path': fpath,
                    'threat_type': ttype or 'blocked',
                    'severity': f.get('severity', ''),
                    'rule': f.get('rule', ''),
                    'reason': f.get('reason', ''),
                    'ml_score': f.get('ml_score', 0),
                })
        # Also add blocked files from the registry that aren't in last_report
        blocked_files = ag.get('blocked_files') or []
        if isinstance(blocked_files, dict):
            blocked_items = blocked_files.items()
        else:
            blocked_items = [(p, '') for p in blocked_files]
        for bpath, binfo in blocked_items:
            if not bpath or bpath in seen_paths:
                continue
            seen_paths.add(bpath)
            bttype = 'blocked'
            if isinstance(binfo, dict):
                bttype = (binfo.get('threat_type') or 'blocked').lower()
            elif isinstance(binfo, str):
                bttype = binfo.lower() if binfo else 'blocked'
            blocked_findings.append({
                'hostname': host,
                'device_id': device_id,
                'path': bpath,
                ...
                'severity': '',
                'rule': '',
                'reason': 'Blocked (from registry)',
                'ml_score': 0,
            })
    return render_template('quarantine.html', quarantined_files=files,
                           agent_quarantined_files=agent_qfiles,
                           blocked_findings=blocked_findings,
                           agent_count=len(agents),
                           quarantine_log=_cloud_quarantine_log_text(), session=session)


@cloud_bp.route('/api/agent-quarantine/list', methods=['POST'])
@_require_login
def cloud_agent_quarantine_list():
    """Send list_quarantine command to all connected agents."""
    agents = _all_agents()
    sent = 0
    for device_id in agents:
        pending = commands.get(device_id, [])
    ...
    return jsonify({'ok': True, 'sent': sent,
                    'message': f'Quarantine list requested for {sent} agent(s).'
                              if sent else 'No agents connected.'}), 200


@cloud_bp.route('/api/agent-quarantine/restore', methods=['POST'])
@_require_login
def cloud_agent_quarantine_restore():
    """Send restore_quarantine command to a specific agent."""
    data = request.get_json(force=True, silent=True) or {}
    device_id = (data.get('device_id') or '').strip()
    filename = (data.get('filename') or '').strip()
    destination = (data.get('destination') or '').strip()
    if not device_id or not filename:
        return jsonify({'ok': False, 'message': 'device_id and filename required.'}), 400
    agents = _all_agents()
    if device_id not in agents:
        return jsonify({'ok': False, 'message': 'Agent not connected.'}), 404
    pending = commands.get(device_id, [])
    pending.append({'action': 'restore_quarantine', 'filename': filename, 'destination': destination})
    commands[device_id] = pending
    return jsonify({'ok': True, 'message': f'Restore command queued for {filename}.'}), 200


@cloud_bp.route('/api/agent-quarantine/delete', methods=['POST'])
@_require_login
def cloud_agent_quarantine_delete():
    """Send delete_quarantine command to a specific agent."""
    data = request.get_json(force=True, silent=True) or {}
    device_id = (data.get('device_id') or '').strip()
    filename = (data.get('filename') or '').strip()
    if not device_id or not filename:
    ...
    if device_id not in agents:
        return jsonify({'ok': False, 'message': 'Agent not connected.'}), 404
    pending = commands.get(device_id, [])
    pending.append({'action': 'delete_quarantine', 'filename': filename})
    commands[device_id] = pending
    return jsonify({'ok': True, 'message': f'Delete command queued for {filename}.'}), 200


@cloud_bp.route('/api/agent-quarantine/delete-all', methods=['POST'])
@_require_login
def cloud_agent_quarantine_delete_all():
    """Send delete_all_quarantine command to all connected agents.
    Permanently deletes ALL quarantined files on every agent."""
    agents = _all_agents()
    if not agents:
        return jsonify({'ok': False, 'message': 'No connected agents.'}), 404
    sent = 0
    for device_id in agents:
        pending = commands.get(device_id, [])
        pending.append({'action': 'delete_all_quarantine'})
        commands[device_id] = pending
        sent += 1
    return jsonify({'ok': True, 'sent': sent,
                    'message': f'Delete-all command sent to {sent} agent(s). All quarantined files will be permanently deleted.'}), 200
...
@cloud_bp.route('/quarantine/list', methods=['GET'], endpoint='quarantine_list')
@cloud_bp.route('/quarantine_list.html', methods=['GET'])
def cloud_quarantine_list():
    files = _cloud_list_quarantine_files()
    return render_template('quarantine_list.html', quarantined_files=files, session=session)


@cloud_bp.route('/quarantine-manage', methods=['GET', 'POST'], endpoint='quarantine_manage')
@cloud_bp.route('/quarantine_manage.html', methods=['GET', 'POST'])
def cloud_quarantine_manage():
    message = None
    error = None
    if request.method == 'POST':
        action = request.form.get('action', '').strip()
        filename = request.form.get('filename', '').strip()
        if not _is_safe_quarantine_path(filename):
            error = 'Invalid filename'
        elif action == 'restore':
            qdir = _cloud_quarantine_dir()
            src = os.path.join(qdir, filename)
            dest = request.form.get('destination', '').strip()
            if not dest:
                # Try to find original path from the log
                files = _cloud_list_quarantine_files()
                entry = next((f for f in files if f.get('filename') == filename), {})
                dest = entry.get('original_path', '')
            if not dest:
                dest = os.path.join(os.path.expanduser('~'), 'Desktop', filename.replace('.enc', ''))
            try:
                os.makedirs(os.path.dirname(os.path.abspath(dest)), exist_ok=True)
            except Exception:
                pass
            if _cloud_decrypt_file(src, dest):
                try:
                    os.remove(src)
                except Exception:
                    pass
                ...
                error = 'Failed to decrypt/restore file'
        elif action == 'delete':
            qdir = _cloud_quarantine_dir()
            src = os.path.join(qdir, filename)
    ...
    files = _cloud_list_quarantine_files()
    return render_template('quarantine_manage.html', files=files,
                           message=message, error=error, session=session)


# -- Quarantine action endpoints (called via fetch from the templates) --

@cloud_bp.route('/restore_file', methods=['POST'])
def cloud_restore_file():
    """Restore a quarantined file by decrypting it (form-data, used by quarantine.html)."""
    try:
        ...
        if not _is_safe_quarantine_path(file_path):
            return jsonify({'success': False, 'error': 'Invalid quarantine path'}), 400
        ...
        if is_encrypted:
            if not _cloud_decrypt_file(file_path, destination):
                return jsonify({'success': False, 'error': 'Failed to decrypt file'}), 500
        else:
            import shutil
            shutil.copy2(file_path, destination)

        # Remove the quarantine payload + sidecar after successful restore.
...
@cloud_bp.route('/quarantine/delete/<path:filename>', methods=['POST'])
def cloud_delete_quarantine_file(filename):
    """Delete a single quarantined file (used by quarantine.html)."""
    try:
        qdir = _cloud_quarantine_dir()
        # filename may be URL-encoded; unquote it.
        filename = unquote(filename)
        if not _is_safe_quarantine_path(filename, qdir):
            return jsonify({'status': 'error', 'error': 'Invalid filename'}), 400
        file_path = os.path.join(qdir, os.path.basename(filename))
        sidecar = file_path + '.json'
        if not os.path.exists(file_path):
...
@cloud_bp.route('/quarantine/delete_all', methods=['POST'])
def cloud_delete_all_quarantine():
    """Delete every file in the quarantine folder (used by quarantine.html)."""
    try:
        qdir = _cloud_quarantine_dir()
        ...
        for filename in os.listdir(qdir):
            ...
            if filename.endswith('.json') or not os.path.isfile(fp):
                continue
            try:
                os.remove(fp)
                deleted += 1
...
@cloud_bp.route('/quarantine/release', methods=['POST'])
def cloud_quarantine_release():
    """Restore a quarantined file (JSON body, used by quarantine_list.html)."""
    try:
        data = request.get_json(force=True, silent=True) or {}
        filename = (data.get('filename') or '').strip()
        destination = (data.get('destination') or '').strip()
        ...
        if not _is_safe_quarantine_path(filename, qdir):
            return jsonify({'status': 'error', 'error': 'Invalid filename'}), 400
        src = os.path.join(qdir, os.path.basename(filename))
        if not os.path.exists(src):
            return jsonify({'status': 'error', 'error': 'File not found'}), 404
        if not destination:
            files = _cloud_list_quarantine_files()
            entry = next((f for f in files if f.get('filename') == os.path.basename(filename)), {})
            destination = entry.get('original_path', '')
        if not destination:
            destination = os.path.join(os.path.expanduser('~'), 'Desktop',
                                       os.path.basename(filename).replace('.enc', ''))
        try:
            os.makedirs(os.path.dirname(os.path.abspath(destination)), exist_ok=True)
        except Exception:
            pass
        if src.lower().endswith('.enc'):
            if not _cloud_decrypt_file(src, destination):
                return jsonify({'status': 'error', 'error': 'Failed to decrypt'}), 500
        else:
            import shutil
        ...
        try:
            ...
            pass
        return jsonify({'status': 'success', 'message': f'Restored to {destination}'})
    except Exception as e:
        return jsonify({'status': 'error', 'error': str(e)}), 500


@cloud_bp.route('/quarantine/delete', methods=['POST'])
def cloud_quarantine_delete_json():
    """Delete a quarantined file (JSON body, used by quarantine_list.html)."""
    try:
        data = request.get_json(force=True, silent=True) or {}
        filename = (data.get('filename') or '').strip()
        ...
        if os.path.exists(sidecar):
            try:
                ...
                pass
        return jsonify({'status': 'success', 'message': 'File deleted from quarantine'})
    except Exception as e:
        return jsonify({'status': 'error', 'error': str(e)}), 500
...
# NOTE: License generation is ONLY via Stripe payment (/purchase-success).
# Admins can list, revoke, and delete licenses but cannot generate free keys.

@cloud_bp.route('/api/license/list', methods=['GET'])
@_require_admin
def license_list():
    """List all licenses (admin only)."""
    if not _license_manager:
        return jsonify({'error': 'License system not initialized'}), 500
    return jsonify({'licenses': _license_manager.list_licenses()})


@cloud_bp.route('/api/license/<license_id>', methods=['GET'])
@_require_admin
def license_info(license_id):
    """Get full info for a single license (admin only)."""
    if not _license_manager:
        return jsonify({'error': 'License system not initialized'}), 500
    info = _license_manager.get_license_info(license_id)
    if not info:
        return jsonify({'error': 'License not found'}), 404
    return jsonify(info)


@cloud_bp.route('/api/license/<license_id>/revoke', methods=['POST'])
@_require_admin
def license_revoke(license_id):
    """Revoke a license (admin only)."""
    if not _license_manager:
        return jsonify({'error': 'License system not initialized'}), 500
    result = _license_manager.revoke_license(license_id)
    status = 200 if result['ok'] else 404
    return jsonify(result), status


@cloud_bp.route('/api/license/<license_id>/unrevoke', methods=['POST'])
@_require_admin
def license_unrevoke(license_id):
    """Un-revoke a license (admin only)."""
    if not _license_manager:
        return jsonify({'error': 'License system not initialized'}), 500
    result = _license_manager.unrevoke_license(license_id)
    status = 200 if result['ok'] else 404
    return jsonify(result), status


@cloud_bp.route('/api/license/<license_id>', methods=['DELETE'])
@_require_admin
def license_delete(license_id):
    """Permanently delete a license (admin only)."""
    if not _license_manager:
        return jsonify({'error': 'License system not initialized'}), 500
    result = _license_manager.delete_license(license_id)
    status = 200 if result['ok'] else 404
    return jsonify(result), status
...
def create_cloud_app():
    ...
    app.secret_key = os.environ.get('CLOUD_SECRET_KEY', secrets.token_hex(32))
