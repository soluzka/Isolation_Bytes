        ...
        try:
            if system == 'windows':
                ...
                if getattr(sys, 'frozen', False):
                    ...
                    exe = sys.executable
                    cmd_str = subprocess.list2cmdline([exe, '--server', self.server_url, f'--key={self.api_key}'])
                else:
                    ...
                    cmd_str = subprocess.list2cmdline([exe, script, '--server', self.server_url, f'--key={self.api_key}'])
                if enable:
...
'''
    ...
    def _check_for_update(self):
        ...
        try:
            ...
            restart_cmd = subprocess.list2cmdline([current_exe, '--server', self.server_url, f'--key={self.api_key}', '--auto-start'])
            bat = f'''@echo off
