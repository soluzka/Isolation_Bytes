...
try:
    ...
    _sp.run([sys.executable, '-m', 'pip', 'install', 'psutil'],
...
try:
    ...
    import subprocess as _sp
    _sp.run([sys.executable, '-m', 'pip', 'install', 'requests'],
            capture_output=True, timeout=60,
            creationflags=getattr(_sp, 'CREATE_NO_WINDOW', 0))
    import requests
...
DEFAULT_API_KEY = "af5caf8d3d8080f8a8686f72dfa52eb92dad9ef95ebfdcd6f6494cf99ed6b909"
HEARTBEAT_INTERVAL = 10      # seconds between heartbeats
SCAN_INTERVAL = 10           # seconds between scans
MAX_FILES_PER_SCAN = 0       # 0 = unlimited, scan the entire PC
MAX_FILE_SIZE = 50 * 1024 * 1024  # 50 MB
AGENT_VERSION = "1.8.944.0"
...
class StandaloneAgent:
    def __init__(self, server_url, api_key, device_id=None):
        ...
        self.device_id = device_id or f'WIN-{socket.gethostname().upper()[:12]}'
    ...
    def _handle_command(self, cmd):
        ...
        if action == 'restore_quarantine':
            ...
            ok = self._restore_quarantine_file(filename, destination)
            print(f"[CMD] Restore {filename}: {'OK' if ok else 'FAILED'}")
            return
        if action == 'delete_quarantine':
            filename = cmd.get('filename', '')
            ok = self._delete_quarantine_file(filename)
            print(f"[CMD] Delete {filename}: {'OK' if ok else 'FAILED'}")
            return
        if action == 'delete_all_quarantine':
            deleted = 0
            failed = 0
            try:
                ...
                print(f"[CMD] Delete all quarantine error: {e}")
            print(f"[CMD] Delete all quarantine: {deleted} deleted, {failed} failed")
                ...
                for fpath in paths_to_process:
                    ...
                    if os.path.exists(fpath):
                        ...
                        if os.path.exists(blocked_path):
                            ...
                            self._unregister_blocked_file(fpath)
                            q_failed += 1
                print(f"[CMD] Unblock+Quarantine: {quarantined} quarantined, {q_failed} failed")
                # Report updated quarantine list to cloud
                try:
                    qfiles = self._list_quarantine()
                    requests.post(f'{self.server_url}/agent/report',
        ...
        try:
            if system == 'windows':
                ...
                if getattr(sys, 'frozen', False):
                    ...
                    exe = sys.executable
                    cmd_str = f'"{exe}" --server {self.server_url} --key={self.api_key}'
                else:
                    ...
                    cmd_str = f'"{exe}" "{script}" --server {self.server_url} --key={self.api_key}'
                if enable:
                    ...
                    if result.returncode == 0:
                        ...
                        # Fallback to registry if schtasks fails
                        subprocess.run(
...
Exec={exe} {script} --server {self.server_url} --key={self.api_key}
Hidden=false
NoDisplay=false
...
'''
    ...
    def _block_ip(self, ip, reason='Blocked'):
        ...
        try:
            if system == 'windows':
                ...
                if not is_admin:
                    ...
                    try:
                        ctypes.windll.shell32.ShellExecuteW(
                            None, 'runas', 'netsh',
                            f'advfirewall firewall add rule name=IsolationBytes_Block_{ip} dir=out action=block remoteip={ip}',
                            None, 0  # SW_HIDE
                        )
                ...
                import subprocess
                # Add to pf table
                subprocess.run(['pfctl', '-t', 'isolationbytes_blocked', '-T', 'add', ip],
                               capture_output=True, timeout=10)
            ...
            if system == 'windows':
                ...
                if not is_admin:
                    try:
                        ctypes.windll.shell32.ShellExecuteW(
                            None, 'runas', 'netsh',
                            f'advfirewall firewall delete rule name=IsolationBytes_Block_{ip}',
                            None, 0
                        )
                ...
                import subprocess
                subprocess.run(['pfctl', '-t', 'isolationbytes_blocked', '-T', 'delete', ip],
                               capture_output=True, timeout=10)
    ...
    def _scan_directory(self, dirpath, max_files=MAX_FILES_PER_SCAN):
        ...
        for root, dirs, files in os.walk(dirpath):
            for filename in files:
                if (max_files > 0 and scanned >= max_files) or not self._running:
                    break
    ...
    def _quarantine_file(self, filepath):
        ...
        try:
            ...
            dest = os.path.join(QUARANTINE_DIR, f"{h}_{base}")
            ...
            try:
                ...
                try:
                    ...
                    print(f"[QUARANTINE] copy+delete also failed: {copy_err} — trying rename fallback")
                    return self._rename_block_fallback(filepath)
            # Write a metadata sidecar so files can be restored later.
            meta = os.path.join(QUARANTINE_DIR, f"{h}_{base}.meta")
    ...
    def _restore_quarantine_file(self, filename, destination=''):
        """Restore a quarantined file to its original or specified path."""
        try:
            # Prevent path traversal — only allow basenames
            filename = os.path.basename(filename)
            src = os.path.join(QUARANTINE_DIR, filename)
            if not os.path.exists(src) or not src.startswith(QUARANTINE_DIR):
                return False
            dest = destination
            if not dest:
                meta_path = src + '.meta'
            ...
            # Restore permissions on the restored file
            self._unblock_file(dest)
            meta_path = src + '.meta'
            if os.path.exists(meta_path):
                os.remove(meta_path)
    ...
    def _delete_quarantine_file(self, filename):
    ...
    def _check_for_update(self):
        ...
        try:
            ...
            args = f'--server {self.server_url} --key={self.api_key} --auto-start'
            bat = f'''@echo off
...
def main():
    ...
    if args.background:
        ...
        if system == 'windows' and exe.lower().endswith('python.exe'):
            exe = exe.replace('python.exe', 'pythonw.exe')
        script = os.path.abspath(__file__)
        cmd = [exe, script, '--server', args.server, f'--key={args.key}']
        if args.device_id:
